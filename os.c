...
// Structure to track periodic event threads
struct ptcb{
	uint32_t period;
	uint32_t counter;
	void(*thread)();
...
void SetInitialStack(int i){
  // **Same as Lab 2 and Lab 3****
	...
	Stacks[i][STACKSIZE-1] = 0x01000000;		//Thumb bit (PSR has T-bit equal to 1)
...
int OS_AddThreads(void(*thread0)(void),
  ...
  tcbs[5].next = &tcbs[0]; //5 points to 0
	...
	//Initialize blocked and sleeping
...
// ****IMPLEMENT THIS****
	int32_t crit = StartCritical();							//Start critical section
	periodicEvents[events].thread = thread;			//Set the period event thread
	periodicEvents[events].period = period;			//Set the period in which it runs
	periodicEvents[events].counter = period;		//Set the counter to the period
	events++;																		//State the number of events in process
	EndCritical(crit);													//End critical section
...
// ROUND ROBIN, skip blocked and sleeping threads
	...
	while((RunPt->blocked)||(RunPt->sleeping)){
